...
`${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/designs/` (user config directory, not project files).
...
- `mkdir -p ${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/designs/...`
- `$D generate`, `$D variants`, `$D compare`, `$D iterate`, `$D evolve`, `$D check`
- `$GSTACK_BIN/gstack-open-url <board-url>` (fallback for viewing boards when `$B` is not available)
...
_DESIGN_DIR="${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
mkdir -p "$TMP_ROOT"
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output "$TMP_ROOT/gstack-ideal-<dimension>.png"
```
...
|----------------|-------------|-----------|-------|
| [screen name]  | ${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
