...
* SELECTIVE EXPANSION: You are a rigorous reviewer who also has taste. Hold the current scope as your baseline and pressure it hard. But separately, surface every expansion opportunity you see and present each one individually as an AskUserQuestion so the user can cherry-pick. Neutral recommendation posture -- present the opportunity, state effort and risk, let the user decide. Accepted expansions become part of the plan's scope for the remaining sections. Rejected ones go to "NOT in scope."
* HOLD SCOPE: You are a rigorous reviewer. The plan's scope is accepted. Your job is to pressure it hard -- catch likely failure modes, test edge cases, ensure observability, and map error paths while keeping any remaining uncertainty explicit. Do not silently reduce OR expand.
* SCOPE REDUCTION: You are a surgeon. Find the minimum viable version that achieves the core outcome. Cut everything else. Be ruthless.
* COMPLETENESS IS CHEAPER THAN IT USED TO BE: AI coding often compresses implementation time enough that the fuller version may now be the better tradeoff. When evaluating "approach A (full, ~150 LOC) vs approach B (90%, ~80 LOC)", default toward the fuller version only when the extra work is still bounded, the same product thesis is being served, and the added operational surface remains legible. Do not use this rule to smuggle in a different thesis, unrelated surface area, or ornamental scope. "Ship the shortcut" is often legacy thinking from when human engineering time was the bottleneck, but "build everything" is still wrong when the boundary is unclear. Do the fuller job when the fuller job is still actually the job.
...
```
If a design doc exists (from `/office-hours`), read it. Use it as the primary planning input for the problem statement, constraints, and chosen approach, then cross-check it against the current repo and user request. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and $BRANCH from the design doc check above):
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t "$PROJECT_STORE"/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
PROJECT_STORE="${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG"
mkdir -p "$PROJECT_STORE/ceo-plans"
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p "$PROJECT_STORE/ceo-plans/archive"
# For each stale plan: mv "$PROJECT_STORE"/ceo-plans/{old-plan}.md "$PROJECT_STORE"/ceo-plans/archive/
```

Write to `${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
6 hours of human implementation can often compress to ~30-60 minutes in a
favorable AI-assisted flow, but the variance is real. The decisions are still
the same; what changes is the implementation loop speed when the repo,
tooling, and task shape cooperate. Present both scales when discussing effort,
and name uncertainty if the compression assumption is weak for this plan.
...
3. **HOLD SCOPE:** The plan's scope is right. Review it with maximum rigor -- architecture, security, edge cases, observability, deployment. Stress it hard and keep any remaining uncertainty explicit. No expansions surfaced.
...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `${GSTACK_ROOT:-$HOME/.claude/skills/gstack}/review/TODOS-format.md`.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
rm -f "${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG"/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
