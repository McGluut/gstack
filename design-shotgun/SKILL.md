...
Before drafting agent prompts, compute one writable temp root and resolve it to an
absolute path:

```bash
TMP_ROOT="${TMPDIR:-${TMP:-.gstack/tmp}}"
mkdir -p "$TMP_ROOT"
TMP_ROOT_ABS=$(cd "$TMP_ROOT" && pwd)
```
...
Temp output: {absolute writable temp root}/variant-{letter}.png
Final location: {_DESIGN_DIR absolute path}/variant-{letter}.png

Steps:
1. Run: {$D path} generate --brief "{brief}" --output {absolute writable temp root}/variant-{letter}.png
2. If the command fails with a rate limit error (429 or "rate limit"), wait 5 seconds
   and retry. Up to 3 retries.
3. If the output file is missing or empty after the command succeeds, retry once.
4. Copy: cp {absolute writable temp root}/variant-{letter}.png {_DESIGN_DIR}/variant-{letter}.png
...
```
{$D path} evolve --screenshot {_DESIGN_DIR}/current.png --brief "{brief}" --output {absolute writable temp root}/variant-{letter}.png
```

**Why temp-root then copy?** In observed sessions, `$D generate --output <project-store path>`
failed with "The operation was aborted" while generating into a writable temp root succeeded.
This is a sandbox restriction. Always generate into a writable temp root first, then `cp`
into the project design store. On Unix hosts that temp root is often `/tmp/`; on other hosts,
use the equivalent writable temp directory.
...
1. **Never leave final artifacts in `.context/`, `docs/designs/`, or a temp root.** All design artifacts go
