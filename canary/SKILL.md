...
- Prerequisites: a reachable URL, browse daemon availability, and a writable `.gstack/canary-reports/` directory. A home-store path is optional for dashboard logging only.
- Outputs: screenshots, baseline/report files under `.gstack/canary-reports/`, and a live health verdict in the conversation.
- Stop when: the URL is unreachable, browse is unavailable, or the user chooses to investigate, roll back, or dismiss instead of continuing the monitor loop.
- If unavailable: if baseline history or the home-store dashboard path is unavailable, continue as a one-run health check and state which persistence step was skipped.
...
If no `baseline.json` exists, take a quick snapshot now as a same-session reference point.
This is not a true pre-deploy baseline. It can tell you whether the site stays stable during
the monitor window, but it cannot prove a newly deployed change caused a problem if the issue
already existed when monitoring began.
...
After each check, compare results against the baseline (or same-session reference snapshot):
...
**Alert on changes, not absolutes.** A page with 3 console errors in the baseline is fine if
it still has 3. One NEW error is an alert. When you are using a same-session reference instead
of a true pre-deploy baseline, frame alerts as "changed during monitoring" rather than "caused
by this deploy" unless you have independent evidence.
...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
PROJECT_LOG_DIR="${HOME:+$HOME/.gstack/projects/$SLUG}"
[ -n "$PROJECT_LOG_DIR" ] && mkdir -p "$PROJECT_LOG_DIR" || echo "NO_HOME_STORE"
```

If `PROJECT_LOG_DIR` is available, write a JSONL entry there: `{"skill":"canary","timestamp":"<ISO>","status":"<HEALTHY/DEGRADED/BROKEN>","url":"<url>","duration_min":<N>,"alerts":<N>}`. If it is unavailable, note that the dashboard log was skipped and continue.
...
- **Baseline is usually the best available comparator for deploy-level regressions.** Without a true pre-deploy
  baseline, canary becomes a health-and-stability check, not a clean deploy-regression proof.
  Encourage `--baseline`
  before deploying.
