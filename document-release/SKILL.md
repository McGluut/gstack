...
- Prerequisites: a feature branch, readable repo docs, and a valid diff against the base branch. `gh` or `glab` is only needed if a PR/MR body should be updated.
- Outputs: updated documentation files, one docs commit when changes were needed, and an optional PR/MR body refresh.
- Stop when: a version bump, narrative rewrite, section removal, new TODO capture, or another risky documentation decision needs user judgment.
- If unavailable: if PR/MR tooling or a writable temp path is unavailable, still finish the repo-local docs pass and report the skipped PR/MR-body update explicitly.
...
Create a portable temp path first:

```bash
TMP_ROOT="${TMPDIR:-${TMP:-.gstack/tmp}}"
mkdir -p "$TMP_ROOT"
PR_BODY_FILE="$TMP_ROOT/gstack-pr-body-$$.md"
```
...
**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > "$PR_BODY_FILE"
```
...
**If GitLab:**
Read the contents of `$PR_BODY_FILE` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
