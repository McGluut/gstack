...
describe('gstack-telemetry-log', () => {
  ...
  cliTest('creates analytics directory if missing', () => {
    ...
    setConfig('telemetry', 'anonymous');
    run('gstack-telemetry-log', ['--skill', 'qa', '--duration', '50', '--outcome', 'success', '--session-id', 'mkdir-123']);
  ...
  cliTest('branch name with quotes does not corrupt JSON', () => {
    ...
    run('gstack-telemetry-log', ['--skill', 'qa', '--duration', '10', '--outcome', 'success', '--session-id', 'branch-quotes-1']);
...
describe('.pending marker', () => {
  cliTest('finalizes stale .pending from another session as outcome:unknown', () => {
    ...
    run('gstack-telemetry-log', ['--skill', 'qa', '--duration', '50', '--outcome', 'success', '--session-id', 'new-456']);
  ...
  cliTest('does not finalize own session pending marker', () => {
    ...
    run('gstack-telemetry-log', ['--skill', 'qa', '--duration', '50', '--outcome', 'success', '--session-id', 'same-session']);
  ...
  cliTest('tier=off still clears own session pending', () => {
    ...
    run('gstack-telemetry-log', ['--skill', 'qa', '--duration', '50', '--outcome', 'success', '--session-id', 'off-123']);
...
describe('gstack-analytics', () => {
  ...
  cliTest('renders usage dashboard with events', () => {
    ...
    run('gstack-telemetry-log', ['--skill', 'ship', '--duration', '30', '--outcome', 'error', '--error-class', 'timeout', '--session-id', 'a-3']);

    const output = run('gstack-analytics', ['all']);
  ...
  cliTest('filters by time window', () => {
    ...
    const output7d = run('gstack-analytics', ['7d']);
