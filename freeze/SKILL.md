...
- Outputs: a persisted edit boundary in the current state root that blocks Edit and Write outside the chosen path.
...
outside this directory will be denied by the freeze hook. To change the
boundary, run `/freeze` again. To remove it, run `/unfreeze`."
...
The freeze boundary persists through the state file until it is replaced,
cleared, or the state root changes. The hook script reads it on every
...
- To deactivate, run `/unfreeze`; do not assume ending the conversation clears the state file automatically
