...
If no binary is found, tell the user: "Global retro discovery is unavailable here. Install `gstack-global-discover` or ensure it exists in your gstack install." and stop.

Run the discovery:
```bash
TMP_ROOT="${TMPDIR:-${TMP:-.gstack/tmp}}"
mkdir -p "$TMP_ROOT"
$DISCOVER_BIN --since "<window>" --format json 2>"$TMP_ROOT/gstack-discover-stderr"
```

Read the stderr output from `$TMP_ROOT/gstack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
