...
- Prerequisites: a project slug and the learnings store for that project. Repo file access is also needed for prune checks.
- Outputs: recent, search, stats, or export views, or updated learnings after a prune or manual-add flow.
- Stop when: the requested learnings operation is complete, or a prune or update choice is waiting on the user.
- If unavailable: if no learnings exist yet, say so plainly. If file checks or project resolution are incomplete, limit the action to the data you do have and say what was skipped.

---

## Runtime Preflight

Before any Bun-backed stats or parsing flow, verify Bun is available:

```bash
command -v bun >/dev/null 2>&1 && echo "BUN_READY" || echo "BUN_MISSING"
```

If `BUN_MISSING`, stop and say: "This `/learn` stats or export path needs Bun for JSONL parsing. Recent/search can still use the gstack binaries, but Bun-backed summaries must stop here."
...
if [ -f "$LEARN_FILE" ]; then
  ...
  LEARN_FILE="$LEARN_FILE" bun -e "
    const lines = require('fs').readFileSync(process.env.LEARN_FILE, 'utf-8').trim().split('\n').filter(Boolean);
    const seen = new Map();
