...
7. **"Turn it off" / "disable"** → `"$GSTACK_BIN/gstack-config" set question_tuning false`
8. **"Turn it on" / "enable"** → `"$GSTACK_BIN/gstack-config" set question_tuning true`
...
"$GSTACK_BIN/gstack-developer-profile" --profile > "$_PROFILE_JSON"
