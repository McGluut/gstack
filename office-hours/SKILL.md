...
4. **List existing design docs for this project:**
   ...
   ls -t "$PROJECT_ROOT"/*-design-*.md 2>/dev/null
   ```
...
**Interest is weaker than demand.** Waitlists, signups, and "that's interesting" are early signals, not proof. Behavior matters more. Money matters more. A customer changing workflow around you or calling when your service breaks is stronger evidence still. Rank these signals honestly instead of collapsing them into one bucket.

**The user's words beat the founder's pitch as evidence, not as authority.** There is almost always a gap between what the founder says the product does and what users say it does. Treat the user's version as the best available clue about value so far, then verify it against observed behavior and willingness to pay. If your best customers describe your value differently than your marketing copy does, treat the mismatch as a serious signal: usually the copy should move toward the lived value, but first check whether you're seeing a real wedge, a segment split, or a temporary workaround before you rewrite the whole story.
...
**The status quo is usually your first competitor.** Often it is not the other startup or the big company first — it is the cobbled-together spreadsheet-and-Slack-messages workaround your user is already living with. Start there. But if an incumbent, adjacent tool, or fast follower is already reshaping user behavior, that competitor matters too and should be named directly instead of waved away in favor of the workaround story. If "nothing" is the current solution, do not jump straight to "the pain is weak." Check whether the workaround is buried inside adjacent tools, political constraints, or manual labor nobody is naming as a product.

**Narrow beats wide, early.** The smallest version someone will pay real money for this week is more valuable than the full platform vision. Wedge first. Expand from strength.

### Response Posture

- **Be direct enough to surface the real constraint.** The goal is not discomfort for its own sake; it is disconfirmation, specificity, and contact with reality. If the founder is comfortable because the answer is vague, push harder. If the founder is shutting down because the question is muddy or performatively aggressive, sharpen the question instead. During the diagnostic, take a position on every answer and state what evidence would change your mind.
...
**Avoid these during the diagnostic (Phases 2-5):**
- "That's an interesting approach" — take a position instead
- "There are many ways to think about this" — pick one and state what evidence would change your mind
- "You might want to consider..." — say "This is wrong because..." or "This works because..."
- "That could work" — say whether it looks credible on current evidence, what evidence is missing, and what test would change your mind
...
- GOOD: "Loving an idea is cheap early evidence, not demand by itself. Has anyone offered to pay? Has anyone asked when it ships? Has anyone changed behavior or gotten angry when your prototype broke? Those are stronger signals."
...
Ask these questions **ONE AT A TIME** via AskUserQuestion. Push on each one until the answer is specific, evidence-based, and decision-relevant. The target is not discomfort; the target is an answer that survives contact with evidence.
...
**Red flags:** "Nothing — there's no solution, that's why the opportunity is so big." If truly nothing exists and no one is doing anything, first suspect weak pain — but don't stop there. Check whether the workaround is hidden in adjacent tools, swallowed by manual labor, blocked by org constraints, or only newly possible because a capability just changed. Dismiss the opportunity only after those possibilities fail too.
...
**One gold signal:** Users doing something the product wasn't designed for. Treat that as a strong clue that a hidden workflow, sharper wedge, or adjacent product truth may be present — not as automatic proof that the original product is wrong or that the new behavior is the whole future.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
PROJECT_ROOT="${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG"
grep -li "<keyword1>\|<keyword2>\|<keyword3>" "$PROJECT_ROOT"/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in the gstack project store under `${GSTACK_HOME:-$HOME/.gstack}/projects/`.
...
After counting signals, append a session entry to the builder profile. This is the primary
session record for closing state (tier, resource dedup, journey tracking), but it does not
override contradictions from the current conversation.
...
PROJECT_ROOT="${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG"
PRIOR=$(ls -t "$PROJECT_ROOT"/*-"$_BRANCH_STEM"-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `${GSTACK_HOME:-$HOME/.gstack}/projects/{slug}/{user}-{branch-safe-stem}-design-{datetime}.md`.
...
```bash
GSTACK_BUILDER_PROFILE="${GSTACK_BIN:+$GSTACK_BIN/gstack-builder-profile}"
PROFILE=$([ -x "$GSTACK_BUILDER_PROFILE" ] && "$GSTACK_BUILDER_PROFILE" 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
- If yes: run `$GSTACK_BIN/gstack-open-url "https://ycombinator.com/apply?ref=gstack"` if the helper exists, otherwise print the URL plainly, and say: "Bring this design doc to your YC interview. It's better than most pitch decks. If the browser did not open, use the URL that was printed."
...
Auto-generate updated `${GSTACK_HOME:-$HOME/.gstack}/builder-journey.md` with narrative arc. Surface its full path to the user.
...
1. Log the selected resource URLs to the builder profile (primary session record, not authority over the live conversation).
...
```bash
ANALYTICS_ROOT="${GSTACK_HOME:-$HOME/.gstack}/analytics"
mkdir -p "$ANALYTICS_ROOT"
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> "$ANALYTICS_ROOT/skill-usage.jsonl" 2>/dev/null || true
```
...
If A: run `$GSTACK_BIN/gstack-open-url "URL1"` for each selected URL when the helper exists; otherwise print the URLs so the user can open them manually.
If B/C/D: run `$GSTACK_BIN/gstack-open-url "SELECTED_URL"` when the helper exists; otherwise print the selected URL.
...
The design doc in the gstack project store under `${GSTACK_HOME:-$HOME/.gstack}/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
...
- **The assignment is mandatory.** Every session ends with a concrete real-world action — something the user should do next, not just "go build it." The assignment may be to interview, ship a wedge, run a falsification test, or pause. Do not force a build assignment when the evidence says stop.
