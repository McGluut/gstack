...
description: |
  ...
  Two modes: daily (low-noise, 8/10 confidence gate) and comprehensive (monthly deep
  scan, lower-confidence exploratory pass). Trend tracking across audit runs.
...
- `/cso --comprehensive` — monthly deep scan (all phases, exploratory 2/10 bar — surfaces more tentative leads)
...
2. If `--comprehensive` → run ALL phases 0-14, comprehensive mode (2/10 exploratory confidence gate). Combinable with scope flags.
...
**Comprehensive mode (`/cso --comprehensive`):** 2/10 exploratory confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include lower-confidence issues that may warrant manual follow-up. Flag these as `TENTATIVE` and keep them clearly separate from confirmed findings.
...
- **Mode discipline matters.** Daily mode optimizes for low-noise, high-confidence findings. Comprehensive mode is allowed to surface more tentative leads, but it must keep them explicitly labeled and separated from confirmed findings.
...
- **Confidence gate is mode-specific.** Daily mode: below 8/10 = do not report. Comprehensive mode may include lower-confidence items only when they are clearly labeled `TENTATIVE` and not presented as proven vulnerabilities.
