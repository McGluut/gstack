...
description: |
  ...
  and optionally quality via LLM judge. Helps compare which model looks best
  for this prompt or skill run using measured tradeoffs instead of vibes.
  Separate from /benchmark, which measures web page performance. Use when:
...
- Outputs: one benchmark table in the conversation and an optional saved JSON result under `${GSTACK_HOME:-$HOME/.gstack}/benchmarks/`.
...
```bash
BIN="${GSTACK_BIN:+$GSTACK_BIN/gstack-model-benchmark}"
...
- **Simplify:** "The quality judge scores each model's output on a 0-10 scale using Anthropic's Claude as one additional comparison signal. Adds ~$0.05/run. Recommended if you care about output quality, not just latency and cost."
...
- **Highest judge-scored output** (if `--judge` ran) - provider with the highest judge score in this run.
- **Best-fit tradeoff for this run** - use judgment. If judge ran: treat the judge score as one signal alongside cost, latency, and prompt fit. Name which tradeoff is winning for this run, and keep any close call or uncertainty explicit. Otherwise: state the tradeoff the user still needs to choose between.
